...
class RobotController:
    ...
    def update_steering_and_speed(self, speed_adjustment, steering_adjustment):
        speed_delta = 0.2
        steer_delta = 0.01

        speed = max(MIN_SPEED, min(MAX_SPEED, self.last_speed + speed_adjustment * speed_delta))
        steering = max(MIN_STEERING, min(MAX_STEERING, self.last_steering + steering_adjustment * (pow(self.last_steering, 2) + steer_delta)))
...
class LineFollowingEnv(gym.Env):
    def __init__(self, robot_controller, use_camera=True):
        ...
        self.action_space = spaces.Discrete(6)  # Discretized actions
        self._map_actions()
    ...
    def step(self, action):
        ...
        self.robot_controller.update_steering_and_speed(speed_adjustment, steering_adjustment)
    ...
    def _calculate_reward(self, observation):
        ...
        reward -= pow(track_offset_from_middle, 2) # deviation from line reward
        reward -= pow(track_direction, 2) # deviation from line reward
        # reward += (speed - 2) * 0.5 # Speed reward
...
if __name__ == "__main__":
    ...
    config = {
        "buckets_sizes": [[0.5, 0.5], [0.15, 0.15, 0.1, 0.08, 0.04, 0.08, 0.1, 0.15, 0.15], [0.05, 0.45, 0.5]],
        "lower_bounds": [-0.5, -1, 1],
        "upper_bounds": [0.5, 1, 3],
        "epsilon_max": 0.8,
        "epsilon_min": 0.05,
        "epsilon_decay": 0.05,
        "alpha_max": 0.3,
